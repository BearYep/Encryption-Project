...
class FibraEncryption:
    ...
    def matrix_C(self, matrix):  # 整個矩陣向右循環 2 位
        num_rows = len(matrix)
        for i in range(num_rows):
            matrix[i] = matrix[i][2:] + matrix[i][:2]  # 將每一行的元素整體向右循環 2 位
        return matrix

    def invert_matrix_C(self, matrix):
        num_rows = len(matrix)
        for i in range(num_rows):
            matrix[i] = matrix[i][-2:] + matrix[i][:-2]  # 將每一行的元素整體向左循環 2 位
        return matrix

    def matrix_D(self, matrix):  # 每row向右循環 仿AES
    ...
    def invert_matrix_D(self, matrix):  # 反向row循環 仿AES
        num_cols = len(matrix[0])
        for i in range(num_cols):
            shift = i
            matrix = [row[-shift:] + row[:-shift] for row in matrix]
    ...
    def encrypt(self, plaintext, key):
        ...
        for i in range(len(key_list)):
            if key_list[i] % 2 == 1: # 奇數轉置
                ...
                cipher_matrix = self.matrix_C(cipher_matrix) # 將每一行的元素整體向右循環 2 位
            else: # 偶數水平鏡射
                cipher_matrix = self.matrix_B(cipher_matrix)
                cipher_matrix = self.matrix_D(cipher_matrix) # 每row向右循環
        # 將加密後的矩陣轉換回加密後的文字
        cipher_text =''
    ...
    def decrypt(self, ciphertext, key):
        ...
        for i in reversed(range(len(key_list))):
            if key_list[i] % 2 == 1: # 奇數轉置 
                cipher_matrix = self.invert_matrix_C(cipher_matrix) # 將每一行的元素整體向右循環 2 位
                cipher_matrix = self.matrix_A(cipher_matrix)
            else: # 偶數水平鏡射 
                cipher_matrix = self.invert_matrix_D(cipher_matrix) # 反向rows循環
                cipher_matrix = self.matrix_B(cipher_matrix)
...
# def test(num_tests=100): # test
    ...
    #     cipher_text =''
    #     decrypted_text =''
    #     key = ''.join(random.choices('abcdefghijklmnopqrstuvwxyz', k = 30))
    #     Encryotor = FibraEncryption()
    #     plaintext = ''.join(random.choices('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k = 300))
    #     # Encrypt
    ...
    # print (cipher)
    # decrypted_plaintext = Encryotor.decrypt(cipher, key)
